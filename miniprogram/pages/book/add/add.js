  ...
  return {
    ...
    loadBill: function () {
      ...
      get(config.api.ledgerBillDetail(this.data.billId), {}).then(function (bill) {
        ...
        that.setData({ loadingBill: false })
      })
    },
    ...
    openCustomerModal: function () {
      var feature = editing ? '修改账单选择客户' : '开单选择客户'
      wx.navigateTo({
    ...
    submit: function () {
      ...
      var feature = editing ? '修改账单' : '新建账单'
      if (!this.data.customerId) {
        console.warn('[账单保存校验失败]', { feature: feature, reason: '未选择客户', billId: this.data.billId || null })
        wx.showToast({ title: '请选择客户', icon: 'none' })
        return
      }
      if (!this.data.items.length) {
        console.warn('[账单保存校验失败]', { feature: feature, reason: '没有商品明细', billId: this.data.billId || null })
      ...
      if (invalidIndex >= 0) {
        console.warn('[账单保存校验失败]', {
          ...
          quantity: items[invalidIndex].quantity,
          unitPrice: items[invalidIndex].unitPrice
        })
      ...
      var saveRequest = editing
        ? put(config.api.ledgerBillUpdate(this.data.billId), payload)
        : post(config.api.ledgerBillAdd, payload)

      this.setData({ submitting: true })
      saveRequest.then(function (bill) {
        that.setData({ submitting: false })
        wx.showToast({ title: editing ? '账单已修改' : '账单已保存', icon: 'success' })
        if (editing) {
          setTimeout(function () {
            wx.navigateBack({
              fail: function (err) {
        ...
        that.setData({ customerId: '', customerName: '', items: [], remark: '', totalAmount: '0.00' })
